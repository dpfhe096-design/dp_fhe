...
Ciphertext<DCRTPoly> DP_GD_clipping::fit(
   ...
   //setup the folder for saving ct_weights
    ...
    const uint32_t resume_iter = 900;
    Ciphertext<DCRTPoly> weights;
    
    std::string weights_enc_path = checkpointDir + "/weights_enc_clip"+"/weights_iter_"+std::to_string(resume_iter)+".bin";
    ...
    for (uint32_t t = start_t; t < T; ++t) {
        ...
        if ((t + 1) >=T-50 || (t + 1)%50==0) {
            ...
            std::string weightsSaveDir = checkpointDir + "/weights_enc_clip"; 
